...
def sh_compare(wb, d, refs):
    ...
    items = [
        ...
        ("B", "KTOS ADTV90 (USD)", f"='12 계산 P10'!B{P10R+6}", ktos, "#,##0.000000"),
        ("C", "ADTV90 분모 (개장일수)", "=90", 90, "#,##0"),
        ("D", "미국 P10 하한 (USD)", f"='12 계산 P10'!B{P10R+2}", p10, "#,##0.000000"),
        ...
        ("I", "최대 비중 (%)", f"='21 계산 비중'!D{refs['wmax']}", wmax * 100, "0.000000"),
        ("J", f"{INDEX_TEST} 지수 레벨", f"='22 계산 지수'!D{IDXR}", idx, "#,##0.000000"),
        # 단위는 09 대조표 선언(%)에 맞춘다. 소수로 두면 수기값(-77.26)과 축이 어긋나 판정이 깨진다.
        ("K", "04-13 조정 전 수익률 (%)", f"='30 계산 분할'!D{SPR}*100",
         (p1 / p0 - 1) * 100, "0.000000"),
