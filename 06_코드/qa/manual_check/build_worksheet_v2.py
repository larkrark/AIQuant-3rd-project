...
def sheet_start(wb, meta):
    ...
    # 2026-07-30 의결로 두 판단(환율·경계일)이 규칙으로 닫혔다. 그 사실을 적어 둔다.
    # 적지 않으면 검산자가 이미 결정된 것을 다시 고민하거나, 반대로 결정 사실을
    # 모르고 다른 값을 넣어 불일치를 자기 실수로 오해한다.
    r = section(ws, r, "적용 규칙 — 2026-07-30 의결분 (파일럿 범위 한정)")
    r = table_head(ws, r, ["항목", "의결 규칙", "범위·유보"])
    for k, v, src in [
        ("환율 적용시점", "평가일 당일 ECOS — 지수·미국 BM 동일 적용",
         "계열코드 공식성 PROVISIONAL"),
        ("산출일", "COMMON_OPEN_DAYS_PILOT (한·미 공통 개장일)",
         "합집합·가격이월은 후속 적용시험"),
        ("연결 방식", "CHAIN_REBASE_PILOT (연결계수)",
         "이번 구간 미발동 — 정확성 실증 아님"),
        ("기업행사", "액면가 기준 5:1 · 경계일 신주권상장일",
         "010120 개별 사건 판정 · 일반화 보류"),
    ]:
        put(ws, r, 1, k, C_GIVEN)
        put(ws, r, 2, v, C_GIVEN)
        put(ws, r, 3, src, C_WARN)
        r += 1
    r += 1

    r = note(ws, r, [
        "■ 의결됐다고 검산이 필요 없어지는 것은 아닙니다 — 확인할 것이 바뀝니다.",
        "  의결 전:  룰북에서 규칙을 직접 도출해 보십시오.",
        "  의결 후:  의결 규칙을 적용하되, 그 규칙이 룰북에서 실제로 도출되는지 확인하십시오.",
        "  도출되지 않으면 그것이 발견사항입니다. 의결됐다는 이유로 넘기지 마십시오.",
        "  환율 항목은 QA 보고(당일 환율만 공표 지수를 재현함)가 의결 근거의 일부였습니다.",
        "  즉 이 항목은 QA 와 완전히 독립적인 확인이 아닙니다. 그 한계를 알고 보십시오.",
    ], fill=C_WARN)
