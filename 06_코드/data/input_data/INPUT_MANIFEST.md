...
각 입력 파일의 생성 주체·원천·상태. QA 스냅샷과 재현성 검증의 기준 문서. 갱신일: 2026-07-31 (bm_kr 출처 확정 D-20260731-01 반영)
...
| `bm_kr.csv` | collect_pilot_inputs.py | **KRX 1028 (KOSPI 200 PR) — 재수집 대조 완료(2026-07-31)·값 동일·OFFICIAL** (경로 증빙: `bm_kr_krx1028_full.csv` 거래대금·상장시총 열, sha256 e4660216…) | 사용 가능 | D-20260731-01. 교체·재산출 불필요(diff 0행) |
...
| `bm_kr_20260723_snapshot.csv` | (구 `bm_kr_krx_official.csv`) | 2026-07-23 수집 스냅샷 — git 계보상 야후 수집본의 동일 복사(C100). 구 명칭은 근거 없는 표기라 정정 | 보관 | 명칭 정정 근거 D-20260731-01 |
| `bm_kr_krx1028.csv` / `bm_kr_krx1028_full.csv` | ingest/fetch_bm_kr.py (KRX 계정) | KRX get_index_ohlcv("1028") 재수집 180행 — full본은 거래대금·상장시가총액 포함(출처 판별 증빙) | 증빙 보관 | bm_kr.csv와 diff 0행 확인(2026-07-31), 커밋 40e166b |
